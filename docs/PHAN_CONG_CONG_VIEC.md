...
| :------------------------------------------- | :------------------------ | :----------------------------------------------------------------------------------------------------- | :------------------- |
| **Role 1: Product Architect**                | `config/test_cases.json`  | Định hướng bài toán & soạn bộ câu test case                                                            | **Vũ Bảo Khánh** |
| **Role 2: Tool Engineer**                    | `src/tools.py`            | Định nghĩa các công cụ (Tools) cho Agent                                                               | **Phạm Đức Hải Triều** |
| **Role 3: Prompt Engineer**                  | `src/prompts.py`          | Viết ReAct System Prompt & phanh Guardrails                                                            | **Võ Hồ Nhật Nam** |
| **Role 4: Core Developer / Integrator**      | `src/app.py`              | **Đầu mối kéo code/file của nhóm (`git pull`), Vibe Code lắp ráp thành App hoàn chỉnh**                | **Võ Hồ Nhật Nam** |
| **Role 5: Observability**                    | `docs/trace_eval.md`      | Lập bảng Scoring Matrix & Soi nhật ký Trace Log                                                        | **Vũ Bảo Khánh** |

*Note: Do nhóm 3 người, **Vũ Bảo Khánh** gộp Role 1 & 5, **Võ Hồ Nhật Nam** gộp Role 3 & 4, **Phạm Đức Hải Triều** đảm nhận Role 2.*

> 🌟 **VAI TRÒ NÒNG NỐT CỦA ROLE 4 (ĐẦU MỐI LẮP RÁP APP HOÀN CHỈNH)**:
>
> - **Role 4 (Võ Hồ Nhật Nam)** đóng vai trò là **Tổ trưởng Lắp ráp**: Sau khi các bạn Role 1, 2, 3 đẩy file lên Git, **Role 4 sẽ gõ `git pull`** để gom toàn bộ dữ liệu về máy.
...
- [ ] **Role 5 (Vũ Bảo Khánh)**: Điền bảng **Scoring Matrix** (chấm 1–5 điểm cho 4 tiêu chí) vào `docs/trace_eval.md`.
- [ ] **Role 2 (Phạm Đức Hải Triều)**: Liệt kê tên các công cụ sẽ tạo trong `src/tools.py` phù hợp với chủ đề nhóm đã chọn.
- [ ] **Role 3 (Võ Hồ Nhật Nam)**: Xác định các trường hợp tool có thể bị lỗi (Failure Modes).
- [ ] **Role 4 (Võ Hồ Nhật Nam)**: Mở Terminal gõ `python src/app.py` kiểm tra xem môi trường sẵn sàng chưa.
...
- [ ] **Role 1 (Vũ Bảo Khánh)**: Viết bộ **Test Cases** vào file `config/test_cases.json` (câu đơn giản, câu multi-step, câu bẫy).
- [ ] **Role 2 (Phạm Đức Hải Triều)**: Dùng AI bổ sung Docstring / Mô tả chuẩn cho các hàm trong `src/tools.py`.
- [ ] **Role 3 (Võ Hồ Nhật Nam)**: Soạn `CHATBOT_BASELINE_PROMPT` trong file `src/prompts.py`.
- [ ] **Role 4 (Võ Hồ Nhật Nam - Đầu mối Lắp ráp)**: Gõ `git pull` để kéo file của Role 1, 2, 3 về máy ➔ Vibe Code nối `run_baseline_chatbot()` trong `src/app.py` và bấm chạy thử.
- [ ] **Role 5 (Vũ Bảo Khánh)**: Ghi lại phản hồi của Chatbot gốc vào `docs/trace_eval.md` (quan sát xem Chatbot có bị ảo giác/không biết thông tin thực tế không).
...
- [ ] **Role 3 (Võ Hồ Nhật Nam)**: Soạn `REACT_SYSTEM_PROMPT` (ép AI sinh Thought -> Action) và đặt `MAX_ITERATIONS (giới hạn số lần lặp)` trong `src/prompts.py`.
- [ ] **Role 2 (Phạm Đức Hải Triều)**: Đảm bảo các hàm trong `src/tools.py` khi gặp lỗi sẽ trả về chuỗi thông báo lỗi chứ không crash code.
- [ ] **Role 4 (Võ Hồ Nhật Nam - Đầu mối Lắp ráp & Vibe App)**: Gõ `git pull` kéo toàn bộ code mới nhất ➔ Vibe Code lắp vòng lặp ReAct Agent Loop hoàn chỉnh trong `src/app.py` và chạy thử nghiệm.
- [ ] **Role 5 (Vũ Bảo Khánh)**: Trích xuất chuỗi `Thought -> Action -> Observation` dán vào `docs/trace_eval.md`.
- [ ] **Role 1 (Vũ Bảo Khánh)**: Kiểm tra xem Agent có vượt qua được câu bẫy (Edge Case) bằng phanh Guardrail hay không.
...
- [ ] 📊 **Role 5B (hoặc Role 5 - Vũ Bảo Khánh)**: Vẽ sơ đồ **Hybrid Flowchart** vào file `docs/hybrid_flowchart.mermaid` thể hiện phân luồng:
